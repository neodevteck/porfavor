...
export default function Dia({ day, rowIdx }) {   
  ...
  const [dayEvents, setDayEvents] = useState([]);
    const {
      setDaySelected,idUsuario ,opcionVista,ActividadesMes,filtrarActividades,colores,usuarios       
  ...
  function ObtenerColorUsuario(evt){  
    ...
    for (let index = 0; index < usuarios.length; index++) {
        if (usuarios.id === evt.USUARIOSID_FLUJOTRABAJO){
      ...
      >
        {dayEvents.map((evt, idx) => (     
          ...
          >          
            ...
            <div className={`h-3 w-3 ${ObtenerColorUsuario(evt)} rounded-full inline-block`}></div>
